    ...
    return (
      <div className="sidebar collapsed">
        ...
        <div className="sidebar-bottom-icons">
          <button className="icon-btn" onClick={openSettingsModal} title="Configuración">
            <SettingsIcon />
          </button>
          <button 
            className={`icon-btn ${isUserConnected ? 'logout' : ''}`} 
            onClick={isUserConnected ? handleLogout : openEmailModal} 
            title={isUserConnected ? "Cerrar Sesión" : "Conectar Correo"}
          >
            {isUserConnected ? <LogOutIcon /> : <MailIcon />}
          </button>
          <button className="icon-profile-btn" title="Perfil / Email">
            {isUserConnected ? 'F' : '?'}
          </button>
        </div>
      </div>
    ...
    <div className="sidebar">
      ...
      {isUserConnected ? (
        ...
        <button className="btn" onClick={openEmailModal}>
          <MailIcon /> Conectar Correo
        </button>
      )}

      <button className="btn" onClick={openSettingsModal} style={{ marginTop: '0.5rem' }}>
